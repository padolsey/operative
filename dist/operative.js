 ...
 * (Browser Support Only, for Node.js, install via NPM)
 * ---
...
(function() {
	...
	var NodeEnvOperativeContext;

	if (isNode) {
		// This isn't pretty but it's the easiest way of reconciling the need to 
		// have this file run in both node and browser envs.
		NodeEnvOperativeContext = operative.Operative = require('./OperativeContext');
		NodeEnvOperativeContext.NodeWorker = require('./contexts/NodeWorker');
	...
	function operative(module, dependencies) {
		...
		var OperativeContext = isNode ? NodeEnvOperativeContext.NodeWorker :
			operative.hasWorkerSupport ? operative.Operative.BrowserWorker : operative.Operative.Iframe;
...
(function() {
	...
	var Promise = Operative.Promise = isNode ? require('bluebird') : window.Promise;
	...
	Operative.prototype = {
		...
		_fixDependencyURLs: function() {
			...
			for (var i = 0, l = deps.length; i < l; ++i) {
				var dep = deps[i];
				if (!/\/\//.test(dep)) {
					deps[i] = dep.replace(/^\/?/, operative.getBaseURL());
				}
			}
		},
